...
public class DES_Encryptor extends DES {
	DES_Key DES_key;
	...
	public DES_Encryptor(String key) {
	...
	String encrypt(String plain_text){
		...
		for(int i=0;i<plain_text.length();i+=8){
			StringBuilder text_64bits = new StringBuilder();
			for(int index=0;index<8;index++){
				if(index+i < plain_text.length()){
					text_64bits.append(conv.fromCharToBinary(plain_text.charAt(index+i) ));
				}else {
					text_64bits.append(conv.fromCharToBinary('?'));
				}
			}
			cipher_text.append(conv.fromBinaryToHexadecimal(new StringBuilder(encrypt_64(text_64bits.toString())  )  ));
		}
		return cipher_text.toString();
	}
	...
	public static void main(String [] args){
		...
		System.out.println(enc.encrypt_64("0000000100100011010001010110011110001001101010111100110111101111"));
		String tmp = enc.encrypt("abdelkarim");
		System.out.println(tmp);
		DES_Decryptor dec = new DES_Decryptor("0001001100110100010101110111100110011011101111001101111111110001");
		System.out.println(dec.decryp(tmp));
//		enc.display();
	}
}
